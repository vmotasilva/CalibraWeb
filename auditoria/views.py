...
def api_iso_avaliacao_salvar_como_padrao(request, auditoria_id):
    ...
    perguntas_origem = list(PerguntaAvaliacaoAuditorIso.objects.filter(auditoria=auditoria, ativa=True).order_by("ordem", "id"))
    if not perguntas_origem:
        return JsonResponse({
            "success": False, 
    ...
    return JsonResponse({
        "success": True,
        "total_salvas": len(perguntas_origem),
        "message": "Modelo Padrão atualizado com sucesso! As novas auditorias e planejamentos futuros utilizarão exatamente este conjunto de perguntas."
    })
...
def avaliacao_portal_publico_view(request, token):
    ...
    context = {
        ...
        "norma_codigo": auditoria.norma.codigo if auditoria.norma else "",
        "norma_nome": auditoria.norma.descricao if auditoria.norma else "",
        "auditor_lider": auditoria.auditor_lider or "Equipe Auditora",
...
def api_iso_analytics_global_data(request):
    ...
    for aud in auditorias_list[:20]:
        ...
        tabela_auditorias.append({
            ...
            "auditor_lider": aud.auditor_lider or "Equipe",
