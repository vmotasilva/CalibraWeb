...
class HistoricoCalibracaoLogicTests(TestCase):
    ...
    def test_result_aprovado_when_eme_leq_ema(self):
        ...
        self.assertIn(
            hist.resultado,
            ["APROVADO_SEM_CORRECAO", "APROVADO_COM_CORRECAO", "REPROVADO"],
        )
...
class ImportInstrumentsTaskTests(TestCase):
    ...
    def test_import_instruments_task_creates_instrumentos(self):
        ...
        job = ImportJob.objects.create(
            filename='test.csv', filepath=tmp.name, status='PENDING'
        )
    ...
    def test_import_instruments_task_maps_all_fields(self):
        ...
        job = ImportJob.objects.create(
            filename='full.xlsx', filepath=tmp.name, status='PENDING'
        )
        ...
        # Unidade e faixa
        unidade = UnidadeMedida.objects.get(nome='LPM')
        self.assertEqual(unidade.nome, 'LPM')
        faixa = FaixaMedicao.objects.filter(instrumento=inst, unidade=unidade).first()
...
class ImportHistoricoTaskTests(TestCase):
    def test_import_historico_task_creates_entries(self):
        ...
        Instrumento.objects.create(tag='HX-01', descricao='Hist Test')
        ...
        job = ImportJob.objects.create(
            filename='hist.xlsx', filepath=tmp.name, status='PENDING'
        )
...
class OcorrenciaTests(TestCase):
    ...
    def setUp(self):
        ...
        self.colaborador = Colaborador.objects.create(
            matricula="500", nome_completo="Test Colab", setor=self.setor
        )
    ...
    def test_ocorrencia_natureza_default(self):
        """Test Ocorrencia keeps natureza unset when not provided"""
        from rh.models import Ocorrencia
        ...
        self.assertEqual(ocor.natureza, "")
...
class SolicitacaoInstrumentoTests(TestCase):
    ...
    def test_solicitacao_instrumento_creation(self):
        ...
        sol = SolicitacaoInstrumento.objects.create(
            solicitante=self.user, tipo="NOVA", motivo="Test reason"
        )
    ...
    def test_solicitacao_instrumento_string_representation(self):
        ...
        sol = SolicitacaoInstrumento.objects.create(
            solicitante=self.user, tipo="NOVA", motivo="Test"
        )
...
class ImportJobTests(TestCase):
    ...
    def test_import_job_creation(self):
        ...
        job = ImportJob.objects.create(
            filename="test.xlsx", filepath="/tmp/test.xlsx", status="PENDING"
        )
    ...
    def test_import_job_status_transitions(self):
        ...
        job = ImportJob.objects.create(
            filename="test2.xlsx", filepath="/tmp/test2.xlsx", status="PENDING"
        )
        job.status = "STARTED"
        job.save()
...
class AvaliacaoFornecedorTests(TestCase):
    ...
    def setUp(self):
        ...
        self.avaliador = Colaborador.objects.create(
            matricula="999", nome_completo="Evaluator", setor=self.setor
        )
...
class QmsImportsTests(TestCase):
    ...
    def test_qms_models_import(self):
        ...
        try:
            ...
            self.fail(f"Failed to import qms.models: {error}")
    ...
    def test_qms_tasks_import(self):
        ...
        try:
            ...
            self.fail(f"Failed to import qms.tasks: {error}")
