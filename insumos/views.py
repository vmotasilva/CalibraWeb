...
def _replace_comentarios_resposta(
    ...
    ComentarioRespostaInsumos.objects.filter(registro=registro).delete()
    ...
    if novos:
        ComentarioRespostaInsumos.objects.bulk_create(novos)


def _build_comentarios_por_pergunta(registro: RegistroAuditoria) -> dict[str, list[str]]:
    result: dict[str, list[str]] = {}
    comentarios = (
        ComentarioRespostaInsumos.objects.filter(registro=registro)
...
def registro_detail(request, pk):
    ...
    if request.method == "POST" and (request.POST.get("action") or "").strip() == "add_question_comment":
        ...
        ComentarioRespostaInsumos.objects.create(
            registro=registro,
            pergunta=pergunta,
            autor=request.user,
            texto=texto,
    ...
    context = {
        ...
        "can_delete_registro": _has_nav_view_access(request.user, "insumos:registro_delete"),
    }
...
def registro_delete(request, pk):
    if request.method != "POST":
        return redirect("insumos:registro_detail", pk=pk)

    if not _has_nav_view_access(request.user, "insumos:registro_delete"):
        messages.error(request, "Acesso negado. Você não tem permissão para excluir registros de insumos.")
        return redirect("insumos:registro_detail", pk=pk)
    ...
    modelo_id = registro.modelo_id
    registro.delete()
    messages.success(request, "Registro de insumos excluído com sucesso.")
    return redirect("insumos:registros_por_modelo", modelo_id=modelo_id)
...
def registros_por_modelo(request, modelo_id):
    ...
    comentarios_resposta_qs = (
        ComentarioRespostaInsumos.objects.filter(pergunta__in=perguntas, registro__in=registros_qs)
