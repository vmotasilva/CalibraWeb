...
# Extract before the blocks loop
top = content.split('{% for bloco in blocos %}')[0]

# Extract after the blocks loop (from modal loading onwards)
bottom_pattern = re.compile(r'<!-- Modal de Loading \(Opcional\) -->.*', re.DOTALL)
bottom_match = bottom_pattern.search(content)
bottom = bottom_match.group(0) if bottom_match else ''

new_loop = '''    <div class="accordion" id="accordionBlocos">
        {% for bloco in blocos %}
        <div class="accordion-item mb-2 border rounded shadow-sm item-card" data-status="{{ bloco.pior_status }}">
            ...
            <div id="collapse{{ bloco.item.id }}" class="accordion-collapse collapse" aria-labelledby="heading{{ bloco.item.id }}" data-bs-parent="#accordionBlocos">
                <div class="accordion-body bg-light">
                    ...
                    <h6 class="fw-bold text-primary mb-3"><i class="bi bi-search me-2"></i>Constatações / Evidências:</h6>
...
new_content = top + new_loop + bottom
